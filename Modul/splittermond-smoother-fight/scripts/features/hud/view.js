import { focusEnabled } from "./focus-context.js";
import { buildFocusedHud, buildFocusedTargetColumn } from "./focus-view.js";
...
import {
    activeDefenseResponseControl,
    continuousActionInterruptionControl,
} from "./response-controls.js";
...
export async function buildHud(context, { movementDistanceCache } = {}) {
    ...
    if (focusEnabled() && !getSetting("minimized", false)) return buildFocusedHud(context, { movementDistanceCache }, { buildActionBar, buildCombatControls, buildGmCheatToggle, buildThemeToggle, buildHudToggle });
...
export function buildTargetColumn(context) {
    if (focusEnabled()) return buildFocusedTargetColumn(context);
...
async function buildPersonalControls(activeContext) {
    ...
    const interruptionControl = continuousActionInterruptionControls(context);
    return `<div class="sf-personal-controls" ${attributes}>
        ...
        ${buildPersonalActionBar(context.actor, `${interruptionControl}${defenseControl}${picker}`, meleeAttackControl)}
    </div>`;
}
...
export function continuousActionInterruptionControls(context) {
    ...
    const request = services.getPendingContinuousActionInterruption?.(context);
    return request ? continuousActionInterruptionControl(request, context.token) : "";
}
