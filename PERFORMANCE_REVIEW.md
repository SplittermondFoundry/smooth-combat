...
- **HUD-Neuaufbau:** `sightRefresh`, Token-, Actor- und Chat-Änderungen können den Aufbau des gesamten HUDs anfordern. Der Scheduler bündelt kurzfristige Anfragen, verhindert aber keine überlappenden asynchronen Aufbauten. Die Rendergeneration verwirft überholte Ergebnisse erst nach `buildHud`. Ob hier im konkreten Spiel spürbar Arbeit verloren geht, sollte ein Laufzeitprofil zeigen.
...
- **Routen und Sichtberechnung:** `updateToken` gleicht weiterhin die Standard-Routenvorschauen ab; `canvasPan` passt die Größe ihrer Segmentmarker an. Die Kosten wachsen mit der Anzahl aktiver Routen und Marker. Foundrys eigene Beleuchtung, Sichtberechnung, Animation und andere Module sind davon getrennte, hier nicht gemessene Kosten.
...
- `npm run check`: vollständige Tests einschließlich Architektur-, Asset- und Regelverträgen.
