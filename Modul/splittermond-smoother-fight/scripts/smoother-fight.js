...
const AUDIO_PROFILE_FLAG = "audioFeedbackProfile";
const AUDIO_CUSTOM_SOUND = "custom";
...
const AUDIO_PROFILE_DEFAULTS = Object.freeze(Object.fromEntries(
    Object.entries(AUDIO_FEEDBACK_EVENTS).map(([id, config]) => [id, { enabled: true, sound: config.defaultSound }])
));
const AUDIO_SOUND_IDS = Object.freeze([...Object.keys(AUDIO_SOUND_PROFILES), AUDIO_CUSTOM_SOUND]);
...
function registerAudioSettingsMenu() {
...
function registerSettingsMenu() {
    ...
    class UserTokenLinksApplication extends Base {
        ...
        async _prepareContext(options) {
            ...
            const sceneTokens = getAllSceneTokens();
            ...
            const tokens = sceneTokens.map((token) => {
                ...
                return {
                    ...
                    name: token.name,
                    displayName,
                    label: `${displayName} — ${token.actor?.name ?? "–"}`,
                    sceneId: scene?.id ?? "",
            ...
            const scenes = Array.from(game.scenes ?? []).map((scene) => ({
                ...
                current: scene.id === canvas?.scene?.id,
            ...
            return {
                ...
                hasDirectTokens: tokens.some((token) => token.isDirect),
            };
        }
...
function assignmentSourceIcon(source) {
    const icons = {
...
function getAudioFeedbackProfile() {
...
async function playFeedbackSelection(soundId, customSound, fallbackSoundId = "shield", force = false) {
